...
const PaymentFlow: React.FC = () => {
  ...
  const { data: pd, isLoading: isPdLoading, isError } = useGetPaymentDetailsForPayerQuery(identifier)
  ...
  const recipientAddress = useMemo(() => {
    ...
    const matched = pd.recipients.find(r => r.network.id === selectedNetwork.id)
  ...
  useEffect(() => {
    ...
    const swapNetwork = pd?.allows_token_swaps && pd.swap_options
      ? {
