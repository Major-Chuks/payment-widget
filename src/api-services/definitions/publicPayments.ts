...
export const publicPaymentsApi = {
  ...
  get_paymentDetailsForPayer: (identifier: string): Promise<get_paymentDetailsForPayer> =>
    apiClient.get(`/api/v1/pay/${identifier}`),


  /** @description Check approval and get approve tx */
  post_checkApprovalAndGetApproveTx: ({ identifier, payload }: { identifier: string, payload: CheckApprovalAndGetApproveTxPayload }): Promise<post_checkApprovalAndGetApproveTx> =>
    apiClient.post(`/api/v1/pay/${identifier}/approve`, payload),


  /** @description Prepare payment transaction */
  post_preparePaymentTransaction: ({ identifier, payload }: { identifier: string, payload: PreparePaymentTransactionPayload }): Promise<post_preparePaymentTransaction> =>
    apiClient.post(`/api/v1/pay/${identifier}/pay`, payload),


  /** @description Submit payment tx hash */
  post_submitPaymentTxHash: ({ identifier, payload }: { identifier: string, payload: SubmitPaymentTxHashPayload }): Promise<post_submitPaymentTxHash> =>
